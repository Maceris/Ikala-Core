...
public class Launcher {
    ...
    private static class LauncherListener implements Listener {
        /**
         * Handle the shutdown event by flipping the shutdown flag. At this point any event handling
         * behavior is undefined, as anything that cares about a shutdown should have been sent the
         * event by now.
         *
    ...
    public static void initialize() {
        ...
        EventManager.getInstance().registerEventListeners(new LauncherListener());
    }
    ...
    public static void main(String[] args) {
        ...
        Launcher.stopCommand =
                SafeResourceLoader.getString("STOP_COMMAND", Launcher.bundle, "stop");
        String stopMessage =
                SafeResourceLoader.getString(
                        "STOP_MESSAGE", Launcher.bundle, "Type '%s' to exit the program\n");
    ...
    private static void mainLoop() {
        ...
        synchronized (Launcher.mainThreadStages) {
            Launcher.mainThreadStages.forEach(stage -> toRemove.add(stage.getId()));
            toRemove.forEach(Launcher::removeMainThreadStage);
        }
    }
     ...
     * @return True if we should continue with the program, false if there was a problem and the
     *     program should exit.
     */
    private static boolean processArguments(final String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            ...
            if ("-l".equalsIgnoreCase(arg) || "--language".equalsIgnoreCase(arg)) {
            ...
            if ("-c".equalsIgnoreCase(arg) || "--country".equalsIgnoreCase(arg)) {
                if (i >= args.length - 1 || Launcher.isFlag(args[i + 1])) {
     ...
     */
    private static boolean setLocale(final String language, final String country) {
        if (null == language) {
            Launcher.printHelp();
            return false;
        }
        ...
        if (index < 0) {
            System.out.println("'" + language + "' is not a valid ISO language.");
            Launcher.printHelp();
            return false;
        }
