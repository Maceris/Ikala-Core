...
/**
 * Utility class for converting the parser output to an abstract syntax tree that we can manipulate.
 *
 * @author Ches Burks
 */
@Slf4j
public class AbstractSyntaxTree {
    ...
    private static Type getType(ReferenceTypeContext node) {
        if (node.classOrInterfaceType() != null) {
            ClassOrInterfaceTypeContext identifier = node.classOrInterfaceType();
        ...
        if (node.arrayType() != null) {
            ...
            if (arrayType.primitiveType() != null) {
                return Type.primitiveArray(
                        AbstractSyntaxTree.getBase(arrayType.primitiveType()), dims);
            }
            if (arrayType.classOrInterfaceType() != null) {
                ClassOrInterfaceTypeContext identifier = arrayType.classOrInterfaceType();
    ...
    private static Type getType(TypeContext node) {
        if (node.primitiveType() != null) {
            return Type.primitive(AbstractSyntaxTree.getBase(node.primitiveType()));
        }
     ...
     * @param currentRoot The current root node of the primary expression, which is updated for each
     *     extension so that each node has an expression on the left and a single operation on the
     *     right.
     * @return The newly created node that will serve as the new root of the expression.
     */
    private static Node primaryExtension(Primary_extensionContext extension, Node currentRoot) {
    ...
    /**
     * Process an additive expression. We know the result will be arithmetic, but need to work out
     * the actual type based on the type of the numbers involved.
     *
     * @param node The context to parse.
    ...
    private static Node process(AdditiveExpressionContext node) {
        if (node.additiveExpression() != null) {
            ...
            result.addChild(AbstractSyntaxTree.process(node.additiveExpression()));
            result.addChild(AbstractSyntaxTree.process(node.multiplicativeExpression()));
            return result;
        }
    ...
    private static Node process(ArgumentListContext node) {
        ...
        node.expression().stream().map(AbstractSyntaxTree::process).forEach(result::addChild);
        return result;
    }
    ...
    private static Node process(AssignmentContext node) {
        ...
        result.setOperator(ExprAssign.Operator.fromText(node.assignmentOperator().getText()));
    ...
    private static Node process(CastExpressionContext node) {
        ...
        if (node.primitiveType() != null) {
            // first case
            result.setType(Type.primitive(AbstractSyntaxTree.getBase(node.primitiveType())));
     ...
     */
    public static CompilationUnit process(@NonNull CompilationUnitContext parserOutput) {
    ...
    /**
     * Process a conditional and expression. This has to be comparing booleans, and thus is a
     * boolean.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
    ...
    private static Node process(ConditionalAndExpressionContext node) {
        if (node.AND() != null) {
            ...
            result.addChild(AbstractSyntaxTree.process(node.conditionalAndExpression()));
            result.addChild(AbstractSyntaxTree.process(node.equalityExpression()));
    ...
    /**
     * Process a conditional expression. We don't know the type offhand for this.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
     */
    private static Node process(ConditionalExpressionContext node) {
        if (node.QUESTION() != null) {
            ...
            result.addChild(AbstractSyntaxTree.process(node.conditionalOrExpression()));
            result.addChild(AbstractSyntaxTree.process(node.expression()));
            result.addChild(AbstractSyntaxTree.process(node.conditionalExpression()));
            return result;
        }
    ...
    /**
     * Process a conditional or expression. This has to be comparing booleans, and thus is a
     * boolean.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
    ...
    /**
     * Process a equality expression. The result is going to be a boolean, regardless of what we are
     * comparing, as long as it's a valid comparison.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
     */
    private static Node process(EqualityExpressionContext node) {
        if (node.equalityExpression() != null) {
            ...
            result.addChild(AbstractSyntaxTree.process(node.equalityExpression()));
            result.addChild(AbstractSyntaxTree.process(node.relationalExpression()));
            result.setType(Type.primitive(Base.BOOLEAN));
            return result;
        }
    ...
    private static Node process(ForStatementContext node) {
        ...
        if (node.statementExpressionList() != null) {
            result.addChild(AbstractSyntaxTree.process(node.statementExpressionList()));
    ...
    private static Node process(LiteralContext node) {
        ...
        if (node.BooleanLiteral() != null) {
            ...
            result.setValue(Boolean.parseBoolean(node.BooleanLiteral().getText()));
            result.setType(Type.primitive(Base.BOOLEAN));
            return result;
        }
    ...
    private static Node process(LocalVariableDeclarationContext node) {
        ...
        for (VariableDeclaratorContext varList :
                node.variableDeclaratorList().variableDeclarator()) {
    ...
    /**
     * Process a multiplicative expression. We know the result will be arithmetic, but need to work
     * out the actual type based on the type of the numbers involved.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
     */
    ...
    /**
     * Process a relational expression. The result is going to be a boolean, regardless of what we
     * are comparing, as long as it's a valid comparison.
     *
     * @param node The context to parse.
     * @return The parsed version of the node.
     */
    ...
    private static Node process(StatementExpressionListContext node) {
        ...
        result.setType(Type.voidType());
        node.statementExpression().stream()
                .map(AbstractSyntaxTree::process)
                .forEach(result::addChild);
        return result;
    }
    ...
    private static Node process(StatementNoShortIfContext node) {
        ...
        if (node.ifThenElseStatementNoShortIf() != null) {
            return AbstractSyntaxTree.process(node.ifThenElseStatementNoShortIf());
        }
    ...
    private static Node process(StatementWithoutTrailingSubstatementContext node) {
        ...
        if (node.expressionStatement() != null) {
            return AbstractSyntaxTree.process(node.expressionStatement().statementExpression());
        }
