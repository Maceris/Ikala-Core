...
public class AVLTreeMap<K extends Comparable<K>, V> {
     ...
     */
    protected BinaryTreeNode<K, V> find(@NonNull K toFind, BinaryTreeNode<K, V> root) {
     ...
     */
    protected BinaryTreeNode<K, V> getSmallestSubnode(BinaryTreeNode<K, V> root) {
        if (root == null) {
            return null;
        }
     ...
     * return value.
     *
     * <p>If the key is already in the tree, updates the value.
     *
    ...
    protected BinaryTreeNode<K, V> insert(BinaryTreeNode<K, V> root, K key, V value) {
         ...
         */
        root.setHeight(
                this.max(this.getHeight(root.getLeft()), this.getHeight(root.getRight())) + 1);
    ...
    /**
     * Inserts the given value into the tree. If the key is already in the tree, updates the value.
     *
    ...
    /**
     * Used to remove a node that has two children, where the node to remove is a left child.
     *
     * @param toRemove The node to remove.
     * @param smallestRight The smallest subnode that is larger than the one we are removing.
     * @see #removeWithTwoChildren(BinaryTreeNode)
     */
    private void removeAsLeftChild(
            BinaryTreeNode<K, V> toRemove, BinaryTreeNode<K, V> smallestRight) {
        ...
        if (toRemove.getLeft() != null && !toRemove.getLeft().equals(smallestRight)) {
            toRemove.getLeft().setParent(smallestRight);
        }
        if (toRemove.getRight() != null && !toRemove.getRight().equals(smallestRight)) {
    ...
    /**
     * Used to remove a node that has two children, where the node to remove is a right node.
     *
     * @param toRemove The node to remove.
     * @param smallestRight The smallest subnode that is larger than the one we are removing.
     * @see #removeWithTwoChildren(BinaryTreeNode)
     */
    private void removeAsRightNode(
            BinaryTreeNode<K, V> toRemove, BinaryTreeNode<K, V> smallestRight) {
    ...
    /**
     * Used to remove a node that has two children, where the node to remove is the root node.
     *
     * @param toRemove The node to remove.
     * @param smallestRight The smallest subnode that is larger than the one we are removing.
     * @see #removeWithTwoChildren(BinaryTreeNode)
     */
    private void removeAsRootNode(
            BinaryTreeNode<K, V> toRemove, BinaryTreeNode<K, V> smallestRight) {
        if (smallestRight.equals(smallestRight.getParent().getLeft())) {
            smallestRight.getParent().setLeft(null);
        } else if (smallestRight.equals(smallestRight.getParent().getRight())) {
    ...
    private void removeWithTwoChildren(BinaryTreeNode<K, V> toRemove) {
        BinaryTreeNode<K, V> smallestRight = this.getSmallestSubnode(toRemove.getRight());
    ...
    protected void updateHeight(BinaryTreeNode<K, V> changed) {
        ...
        int height =
                this.max(this.getHeight(changed.getLeft()), this.getHeight(changed.getRight())) + 1;
