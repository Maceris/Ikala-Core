...
public class TestAVLTreeMap {
    ...
    private void checkBalance(AVLTreeMap<Long, String> map, BinaryTreeNode<Long, String> root) {
    ...
    private <K extends Comparable<K>, V> void checkOrder(BinaryTreeNode<K, V> root) {
        if (root == null) {
            return;
        }
        if (root.getLeft() != null) {
            Assertions.assertTrue(root.getLeft().getKey().compareTo(root.getKey()) < 0);
            this.checkOrder(root.getLeft());
        }
        if (root.getRight() != null) {
            Assertions.assertTrue(root.getRight().getKey().compareTo(root.getKey()) > 0);
    ...
    public void testTreeBalance() {
        ...
        List<Long> cases =
                Arrays.asList(
                        8L, 1L, 6L, 2L, 5L, 4L, 45L, 23L, 7L, 13L, 1112L, 3L, 123L, 145L, 345L,
                        564L);
    ...
    public void testTreeOrdering() {
        ...
        List<String> strings =
                Arrays.asList(
                        "Test", "walrus", "xyzzy", "AaaaAAa", "aaaaaaa", "bbasedf", "static",
                        "null?");
