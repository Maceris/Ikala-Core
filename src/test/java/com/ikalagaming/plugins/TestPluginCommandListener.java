...
class TestPluginCommandListener {
	...
	@Mock
	private PluginCommand intendedCommand;
	@Mock
	private Consumer<List<String>> intendedConsumer;

	@Mock
	private PluginCommand otherCommand;
	@Mock
	private Consumer<List<String>> otherConsumer;
	...
	void setUp() {
		...
		given(manager.getCommands())
			.willReturn(List.of(otherCommand, intendedCommand));
	...
	void testPluginCommandSentWithArguments() {
		var argument = "arg1";
		var event =
			new PluginCommandSent(intendedCommandName, List.of(argument));
