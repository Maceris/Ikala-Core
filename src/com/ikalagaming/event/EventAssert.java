 ...
 * creates listeners as if a normal event listener has been scheduled, so it
 * tests if event listeners should have received events, not just if the event
 * manager tried to fire an event.
 *
 * @author Ches Burks
 *
 */
public class EventAssert {
	private static Map<Class<? extends Event>, EventMonitor<?>> monitors =
		new HashMap<>();
	 ...
	 * @return The number of times since reset or initial listening the event
	 *         has been received.
	...
	/**
	 * Returns a list of events that have been recorded since the listening has
	 * started, or was last reset. If no events were received, or we were not
	 * listening to the event, an empty list will be returned. The returned list
	 * is a copy and can be freely modified.
	 *
	 * @param <T> The type of event.
	 ...
	 */
	public static <T extends Event> List<T>
		getRecordedEvents(Class<T> eventClass) {
		...
		EventMonitor<T> monitor =
			(EventMonitor<T>) EventAssert.monitors.get(eventClass);
	...
	/**
	 * Resets the number of times the given event class has been recorded, and
	 * also clears the list of events that have been recorded for that event
	 * type.
	 *
	 * @param <T> The type of event.
	 ...
	 * for it, or since the last time we reset the count.
	 *
